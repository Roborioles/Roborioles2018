...
// END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTRUCTOR
	...
	if(selectionNum == 1) {
		...
		AddParallel(new AutoElevatorExec(0));
		...
		AddSequential(new AutoElevatorExec(0));
		//AddSequential(new WaitCmd(2)); //meaningless if autoelevatorexec works
		AddSequential(new AutoRotate(-30, .6));
		AddSequential(new WaitCmd(1)); //might be meaningless if autorotate gets fixed??
		AddSequential(new IntakeOpenCmd(true));
		AddSequential(new AutoDrive(2));
		...
		AddParallel(new AutoDrive(7)); //double check this distance
		AddSequential(new PickeyUppy());
		AddSequential(new AutoDrive(-2));
		if(Robot::elevator->CubeDistance() < 9) { //places on switch, please test + debug all of this lmao
			AddSequential(new ElevatorSwitch());
		...
		AddSequential(new ElevatorFloor());
		AddSequential(new AutoElevatorExec(0));
		//no time for pickeyuppy lol, already at like 20 sec probably
	...
	//RIGHT POSITION AUTONOMOUSES
		...
		AddSequential(new ElevatorFloor());
		AddParallel(new AutoElevatorExec(1));
		AddSequential(new AutoDrive(-2));
	...
	//MIDDLE AUTONOMOUSES
		...
		AddSequential(new AutoDrive(4));
		AddSequential(new AutoRotate(90, .6));
